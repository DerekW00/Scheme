...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        return env.make_child_frame(self.formals, args, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        symb=Pair(bindings.first.first, symb)
        val=Pair(scheme_eval(bindings.first.second.first, env),val)
        bindings=bindings.second

    check_formals(symb)

    return env.make_child_frame(symb,val)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        _= MacroProcedure(target.second, expressions.second, env)
        env.define(target.first, _)
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError
